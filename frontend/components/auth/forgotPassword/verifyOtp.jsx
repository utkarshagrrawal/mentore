...
export function VerifyOtp({ forgotPassword, handleChange, setLoading }) {
    ...
    const handleVerify = async (e) => {
        e.preventDefault();
        setLoading(true);
        let verifyOtp = await fetch('https://mentore-backend.vercel.app/user/verifyotp', {
            method: 'POST',
            headers: {
    ...
    const handleResend = async (e) => {
        e.preventDefault();
        setLoading(true);
        let resendOtp = await fetch('https://mentore-backend.vercel.app/user/resendotp', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': localStorage.getItem('token')
                ...
                <form className="space-y-6" onSubmit={handleVerify}>
                    <div>
                        ...
                        <div className="flex justify-end text-sm">
                            <button onClick={handleResend} className="font-medium text-indigo-600 hover:text-indigo-500 cursor-pointer">
                                Resend OTP
                            </button>
                        </div>
                    </div>
