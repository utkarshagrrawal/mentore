...
export default function ChangePasswordBlock({ setLoading }) {
  ...
  let passwordConditions = Object.keys(conditionsList).map((key, index) => {
    return (
      <li key={index} className="text-sm font-semibold text-red-600">
        {conditionsList[key]}
      </li>
    );
  });
  ...
  useEffect(() => {
    setIsButtonDisabled(
      Object.keys(conditionsList).length > 0 ||
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    let changePassword = await fetch(
      "https://mentore-backend.vercel.app/user/change-password",
      {
        method: "PUT",
        headers: {
