...
function Comment({
  ...
  return (
    <>
      <div className="rounded-lg border border-slate-200">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <div
                ...
                onClick={() => handleReplyVisibility(item.comment_id)}
              >
              ...
              {user.current.email === item.user_email && (
                <>
                  ...
                  <div
                    className="hover:cursor-pointer flex items-center"
                    onClick={() => handleEditVisibility(item.comment_id)}
                  >
            ...
            <div className="flex items-center">
              ...
              <span
                className={`text-sm font-semibold ${
                  item.liked_by.length - item.disliked_by.length >= 0
                    ? "text-blue-500"
                    : "text-red-500"
                }`}
              >
                {item.liked_by &&
                  item.disliked_by &&
                  item.liked_by.length - item.disliked_by.length}
              </span>
              <div className="flex items-center">
                {item.disliked_by &&
                item.disliked_by.includes(user.current.email) ? (
                  <svg
                    className="hover:cursor-pointer"
                    xmlns="http://www.w3.org/2000/svg"
                    width={24}
                    height={24}
                    viewBox="0 0 24 24"
                    onClick={() => handleDislike(item.comment_id)}
                  >
                    <path
                      fill="currentColor"
        ...
        <div className="p-4">
          {editFields[item.comment_id]?.showEditInput ? (
            <div className="mt-2 relative">
              <input
                onChange={(event) => handleEditText(event, item.comment_id)}
                value={editFields[item.comment_id]?.editText || ""}
              ...
              <button
                ...
                onClick={() => handleEditPost(item.comment_id)}
              >
                <TbSend
        ...
        {replyFields[item.comment_id]?.showReplyInput && (
          <div className="my-2 relative mx-4">
            <input
              onChange={(event) => handleReplyText(event, item.comment_id)}
              value={replyFields[item.comment_id]?.replyText || ""}
              className="w-full border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none p-1 pr-[45px] resize-none duration-200"
              placeholder="Enter reply here..."
            />
            <button
              className="flex items-center text-white font-semibold p-1 rounded-lg transition duration-200 absolute top-0 right-0"
              onClick={() => handleReplyPost(item.comment_id)}
            >
              <TbSend
                className="text-2xl text-black hover:rotate-45 duration-200 hover:text-blue-500"
                title="Post comment"
      ...
      {item.child_comments &&
        item.child_comments.map((child, id) => {
          return (
