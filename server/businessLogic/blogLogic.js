...
const createBlogLogic = async (body, user) => {
    ...
    const { error } = await supabase
        .from("blogs")
        .insert({ title: title, content: content, email: email, name: name, likes: 0, });
...
const postCommentOnBlogLogic = async (user, params, body) => {
    ...
    const { error } = await supabase
        .from("blog_comments")
        .insert({ blog_id: id, comment: comment, user_email: email, user_name: name, gender: gender });
...
const addReplyOnCommentLogic = async (params, user, body) => {
    ...
    const { error } = await supabase
        .from("blog_comments")
        .insert({ parent_comment_id: commentId, comment: reply, user_email: email, user_name: name, blog_id: blogId, gender: gender });
